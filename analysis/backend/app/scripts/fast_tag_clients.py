...
def fast_tag():
    print("🚀 Starting Fast-Tag Analysis for Clients...")
    with SessionLocal() as db:
        # Clear existing tags first to ensure a clean run
        db.execute(text("UPDATE articles SET client_tags = ARRAY[]::text[]"))
        db.commit()
        
        for client in ALL_CLIENTS:
            # Case-insensitive keyword search in title and summary
            # Using PostgreSQL array_append and distinct to avoid duplicates
            query = text("""
                UPDATE articles 
                SET client_tags = array_remove(array_append(client_tags, :client), NULL)
                WHERE (title ILIKE :pattern OR summary ILIKE :pattern)
                AND NOT (:client = ANY(client_tags))
            """)
            result = db.execute(query, {"client": client, "pattern": f"%{client}%"})
            if result.rowcount > 0:
                print(f"✅ Tagged {result.rowcount} articles for: {client}")
        
        db.commit()
    print("✨ Client Tagging Complete!")
