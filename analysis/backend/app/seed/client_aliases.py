...
# This list matches the frontend constants/clients.ts
ALL_CLIENTS = [
...
def seed_client_aliases():
    with SessionLocal() as db:
        # Check if already seeded
        count = db.query(ClientAlias).count()
        if count > 0:
            print(f"Skipping client aliases seed: {count} already exist.")
            return
        ...
        for client in ALL_CLIENTS:
            ...
            batch.append(ClientAlias(client_name=client, alias=client))
        
        db.bulk_save_objects(batch)
        db.commit()
        print(f"✅ Successfully seeded {len(seen)} unique client aliases.")
